...
export type TrackRecorderStatus =
  | "idle"
...
export interface TrackRecorderState {
  status: TrackRecorderStatus;
  elapsedMs: number;
  errorMessage: string | null;
}

const INITIAL_STATE: TrackRecorderState = {
  status: "idle",
  elapsedMs: 0,
  errorMessage: null,
};
 ...
 * (녹음 결과를 자체적으로 보관하지 않는다 — 즉시 트랙으로 편입된다.)
 */
export function useTrackRecorder(onComplete: (buffer: AudioBuffer) => void) {
  const [state, setState] = useState<TrackRecorderState>(INITIAL_STATE);
